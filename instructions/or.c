    ...
    uint32_t *value1, int *j, char *log)
{
    ...
    if (cb[0] == REG) {
        ...
        *j += snprintf(log, 256, "%s did the instruction OR on the value in r%d = %x",
    ...
    if (cb[0] == DIR) {
        ...
        *j += snprintf(log, 256,"%s did the instruction OR on the value %x",
    ...
    uint32_t *val, int *j, char *log)
{
    ...
    if (cb[1] == IND) {
        ...
        my_memmove(val, &arena->arena[(PC +
            (offset % IDX_MOD)) % MEM_SIZE], REG_SIZE);
        *j += snprintf(&log[*j], 256 - *j," and the value %x",
     *val);
    }
    return 0;
}
    ...
    uint32_t *result, int *j, char *log)
{
    ...
    *result = __builtin_bswap32(*result);
    if (reg >= 1 && reg <= 16) {
        ...
        snprintf(&log[*j], 256 - *j," and store it in r%d\n", reg);
